...
class _MoviesContentState extends State<MoviesContent> {
  ...
  Widget build(BuildContext context) {
    return Expanded(
      child: SignalBuilder(
        builder: (context) {
          final movies = _movieController.moviesList;
          final rentalMovies = _rentalMovieController.rentalMoviesList;

          final rentedMovieIds = rentalMovies.map((m) => m.id).toSet();
          final filteredMovies = movies.where((movie) => !rentedMovieIds.contains(movie.id)).toList();
